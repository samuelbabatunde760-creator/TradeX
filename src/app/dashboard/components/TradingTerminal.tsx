...
const OrderPanel = memo(({ onTrade, loading, message, timeframe, setTimeframe, amount, setAmount, t }: any) => {
  return (
    <div className="glass-panel p-6 rounded-2xl border-white/10">
      <h3 className="font-bold text-lg mb-6 border-b border-white/10 pb-3 flex items-center gap-2">
        <Zap size={18} className="text-brand-blue" /> {t('dashboard.execute')}
      </h3>
      ...
      <div className="space-y-4">
        <div>
          <label className="block text-sm text-brand-silver-dark mb-2">{t('dashboard.amount')}</label>
        ...
        <div>
          <label className="block text-sm text-brand-silver-dark mb-2 flex items-center gap-2">
            <Clock size={14} /> {t('dashboard.timeframe')}
          </label>
      ...
      <div className="text-center mt-4">
          <span className="text-[10px] text-brand-silver-dark uppercase tracking-widest font-bold">{t('dashboard.payout')}: 85% • {t('dashboard.instantExecution') || 'Instant Execution'}</span>
        </div>
      </div>
    </div>
  );
...
const RecentActivity = memo(({ trades, t }: any) => {
  ...
  return (
    <div className="glass-panel p-6 rounded-2xl flex-1 border-white/10">
      <h4 className="text-sm font-bold text-brand-silver-dark uppercase tracking-widest mb-4 flex items-center gap-2">
        <Clock size={14} /> {t('dashboard.openPositions')}
      </h4>
        ...
        {pendingTrades.map((trade: any) => (
          <div key={trade.id} className="p-3 bg-brand-blue/5 rounded-xl border border-brand-blue/20 flex justify-between items-center animate-pulse">
            <div>
              ...
              <div className="text-[10px] text-brand-silver-dark">{trade.timeframe} {t('dashboard.duration')}</div>
            </div>
            <div className="text-right">
              <div className="text-sm font-mono font-bold text-brand-blue">${trade.amount}</div>
              <div className="text-[9px] text-yellow-500 font-bold uppercase tracking-tighter">{t('dashboard.executing')}</div>
            </div>
          </div>
        ))}
...
export default function TradingTerminal() {
  ...
  useEffect(() => {
    ...
    // Delayed initialization to fix "waiting" jank
  ...
  const placeTrade = useCallback(async (direction: 'call' | 'put') => {
    ...
    try {
      ...
      setMessage(`${t('dashboard.orderPlaced')}: $${numAmount} ${direction.toUpperCase()} ${t('dashboard.at')} ${entryPrice.toFixed(2)}`);
      fetchTrades();
      ...
      setTimeout(async () => {
        ...
        setMessage(won ? `🎉 ${t('dashboard.youWon')} ${t('dashboard.profit')}: $${profit.toFixed(2)}` : `📉 ${t('dashboard.tradeClosed')}. ${t('dashboard.result')}: ${t('dashboard.loss')}`);
        fetchTrades();
      }, delay);
  ...
  }, [amount, timeframe, fetchTrades, t]);
    ...
    <div className="flex-1 flex flex-col lg:flex-row gap-6 h-full pb-10">
      ...
      <div className="flex-1 flex flex-col gap-6">
        <div className="flex-1 glass-panel rounded-2xl flex flex-col overflow-hidden min-h-[450px] border-white/10 shadow-2xl">
          <div className="p-4 border-b border-white/10 flex justify-between items-center bg-white/5 backdrop-blur-sm">
            <div className="flex items-center gap-3">
              ...
              <span className="px-2 py-0.5 bg-brand-blue/10 text-brand-blue text-[10px] font-bold rounded uppercase tracking-wider border border-brand-blue/20">{t('common.secure')} {t('common.protocol')}</span>
            </div>
            <div className="flex items-center gap-6">
               <LanguageSwitcher />
               <div className="text-right">
                  <div className="text-[10px] text-brand-silver-dark uppercase font-bold tracking-widest">{t('dashboard.marketPrice')}</div>
        ...
        <div className="hidden lg:block">
           <RecentActivity trades={recentTrades} t={t} />
        </div>
      </div>
      ...
      <div className="w-full lg:w-80 flex flex-col gap-6">
        ...
        <div className="glass-panel p-6 rounded-2xl border-white/10 flex items-center gap-4 bg-brand-blue/5">
          ...
          <div>
            <div className="text-xs font-bold text-white uppercase">{t('dashboard.secureNode')}</div>
            <div className="text-[10px] text-brand-silver-dark">{t('dashboard.sslTerminal')}</div>
          </div>
        </div>
      </div>
    </div>
